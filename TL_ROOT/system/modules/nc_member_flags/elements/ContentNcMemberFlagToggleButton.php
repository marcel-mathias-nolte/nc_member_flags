...
/**
 * Class ContentNcMemberFlagToggleButton
 *
 * Front end content element "MemberFlagToggleButton".
 */
class ContentNcMemberFlagToggleButton extends \ContentElement
{
	...
	protected function compile()
	{
		...
		if ($this->Input->post('FORM_SUBMIT') == 'member_flag_toggle_button' && FE_USER_LOGGED_IN)
		{
			...
			if (is_array($flags))
			{
				...
				if (strlen($this->member_flag_jump_to))
				{
					$objNextPage = $this->Database->prepare("SELECT id, alias FROM tl_page WHERE id=?")->limit(1)->execute($this->member_flag_jump_to);
		...
		$this->Template->slabel = specialchars($this->title);
